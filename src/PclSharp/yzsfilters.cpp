...
float* radius_down_sampling_run(const float* arrayF1, int rows, int* resultRow)
{
...
float* statistical_outlier_filter(const float* array, const int rows, const int meanK, const float stddevMulThresh, int* resultRow)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud;
...
float* smooth_filter(const float* array, const int rows,const int polynomialOrder, const float searchRadius, int* resultRow)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud;
	Helper::Create3DPointCloudFromArray<pcl::PointXYZ>(array, rows, cloud);
	YzsFilters<pcl::PointXYZ> rd;
